...
class PyTrieNode(object):
    ...
    def add(self, seq):
        ...
        Args:
            seq (list): 一个包含键值对的列表，第一个元素为键，剩余元素为值。
    ...
    def find(self, sent):
        ...
        - Tuple[str, bool]: 一个元组，包含两个元素：
            - str: 查找结果，如果找到了，则返回找到的字符串；否则返回空字符串。
            - bool: 是否找到了目标字符串，True表示找到了，False表示没找到。
...
class PyTrie(object):
    ...
    def scan(self, sent):
        """
        根据给定的句子，使用Trie树扫描出句子中的所有单词。

        Args:
            sent (str): 待扫描的句子。

        Returns:
            List[str]: 扫描出的单词列表，如果句子中存在无法识别的字符，则将其以'invalid:字符'的形式返回。

        """
        words = []
        while len(sent) > 0:
            buf, succ = self.root.find(sent)
            # print(f'buf: {buf}, succ: {succ}')
            if succ:
                words.append(buf)
                sent = sent[len(buf):]
            else:
                words.append(f'invalid:{sent[0]}')
                sent = sent[1:]
        return words
